...
Attach files or Choose folder streams selected files to local workspace storage in 1 MB chunks. Files are not loaded wholesale into browser memory, SQLite, or model context. There is no fixed per-file size or ten-file limit on this uploader. The default workspace upload quota is 10 GiB; administrators can configure `EWB_UPLOAD_QUOTA_BYTES`. Free disk space must retain a 128 MiB reserve. Concurrent unfinished uploads reserve their declared sizes.

Questions receive bounded previews of the first ten files. Reviewed Python tasks receive a manifest and read-only mounts of the complete selected files. Scripts must stream large datasets within the worker's 512 MB memory and 60-second execution limits. Upload support does not imply every binary format has a reader; available Python dependencies still determine what can be analyzed. File hashes are checked before execution; changes fail rather than silently altering approved inputs.
